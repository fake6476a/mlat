"""Route Layer 3 correlation groups through the MLAT solver pipeline."""
...
from frisch import _timing_residuals, solve_constrained_3sensor, solve_toa
from gdop import compute_gdop, compute_gdop_2d
from geo import ecef_to_lla, ft_to_m, lla_to_ecef, m_to_ft, sensor_lla_to_ecef
...
# Use the upper-end GDOP threshold because the Cornwall array is nearly coplanar.
MAX_GDOP = 20.0
...
def _solve_with_outlier_rejection(
    ...
    """Solve while iteratively dropping the worst outlier sensor when needed."""
    ...
    for iteration in range(n - min_sensors + 1):
        ...
        # Step 1: Build an initializer for the current sensor subset.
        x0 = None
        method = "unknown"

        if cur_n >= 5:
            # Try a few reference rotations for the Inamdar initializer.
            best_init = None
        ...
        # Step 2: Solve with an adaptive prediction weight.
        ...
        # Step 3: Decide whether the worst sensor should be rejected.
        ...
        # Remove the worst sensor and retry.
...
def solve_group(
    ...
    """Solve one Layer 3 correlation group and return a result or failure reason."""
    icao = group.get("icao", "")
    ...
    # Convert altitude from feet to meters when available.
    altitude_m: float | None = None
    if altitude_ft is not None:
        altitude_m = ft_to_m(altitude_ft)

    # Enforce the minimum sensor requirements for the available constraints.
    ...
    # Extract sensor positions in ECEF and relative arrival times.
    sensor_positions = np.zeros((n_sensors, 3))
    sensor_alts_m = np.zeros(n_sensors)
    arrival_times = np.zeros(n_sensors)

    # Use the earliest reception as the relative-time reference to preserve float precision.
    ...
    for i, rec in enumerate(receptions):
        sensor_positions[i] = sensor_lla_to_ecef(
            rec["lat"], rec["lon"], rec["alt"]
        )
        sensor_alts_m[i] = rec["alt"]
        # Make timestamps relative before converting to float.
        dt_s = rec["timestamp_s"] - ref_timestamp_s
        dt_ns = rec["timestamp_ns"] - ref_timestamp_ns
        arrival_times[i] = dt_s + dt_ns * 1e-9

    # Determine the minimum sensor count allowed during outlier rejection.
    if altitude_m is not None and position_prior_ecef is not None:
    ...
    # Use outlier rejection only for 4+ sensor groups, but never drop into the weak 2-sensor path.
    if n_sensors >= 4:
        ...
        # Use the direct solve path for 2-3 sensor groups.
        x0 = None
        ...
        # Use a stronger prediction weight for 2-sensor solves than for 3+ sensors.
    ...
    # Step 3: Validate the candidate solution.

    # Enforce the maximum sensor range.
    for s in used_sensors:
        if np.linalg.norm(position - s) > MAX_RANGE_M:
            return None, "range_exceeded"

    # Tighten residual thresholds for better-determined systems.
    if n_sensors == 2 and position_prior_ecef is not None:
    ...
    # Keep prior-aided solutions near the cached prior.
    ...
    # Apply GDOP gating, using 2D GDOP for altitude-constrained 3-sensor cases.
    if n_used >= 4:
    ...
    # Convert the result back to WGS84 coordinates.
    lat, lon, alt_m = ecef_to_lla(position[0], position[1], position[2])

    # Preserve the known altitude when available; otherwise use the solved altitude.
    if altitude_ft is not None:
